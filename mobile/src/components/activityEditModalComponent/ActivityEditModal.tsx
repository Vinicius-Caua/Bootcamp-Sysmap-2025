...
import {Activity, ActivityType} from '../../models/activities/activitiesModel';
import CustomDialog from '../dialogComponent/CustomDialog';

interface ActivityEditModalProps {
  visible: boolean;
  onClose: () => void;
  onActivityEdit?: (wasCanceled?: boolean) => void;
...
export default function ActivityEditModal({
  ...
  const selectedTypeIdRef = useRef<string>('');

  const {
    activities: {updateActivity, getActivityTypes, cancelActivity},
  } = useAppContext();

  // Fetch activity types when the modal is visible
  useEffect(() => {
    if (visible) {
  ...
  }, [visible, getActivityTypes]);

  useEffect(() => {
    if (activity && visible) {
      setTitulo(activity.title);
      setDescricao(activity.description);
      setActivityType(activity.typeId || '');
      setIsPrivate(activity.private);
      setImageUri(activity.image);

      // Set and convert the date if it exists
      if (activity.scheduledDate) {
        setData(new Date(activity.scheduledDate));
      }

      // Convert the address to an object if it's a string
      if (activity.address) {
        try {
          const addressData =
  ...
  // Find activity type ID based on the name
  useEffect(() => {
    if (activity?.type && activityTypes.length > 0) {
      const foundType = activityTypes.find(
        type => type.name.toLowerCase() === activity.type.toLowerCase(),
      );

      if (foundType) {
        setActivityType(foundType.id);
        selectedTypeIdRef.current = foundType.id;
      } else {
        console.warn('Tipo não encontrado para:', activity.type);
      }
    }
  ...
  const handleEditActivity = async () => {
    ...
    if (!isValid) {
      Toast.show({
        ...
        text2: 'Todos os campos são obrigatórios para atualizar a atividade',
      });
      return;
    }
    ...
    try {
      if (!activity || !activity.id) {
        throw new Error('ID da atividade não disponível');
      }
  ...
  return (
    <Modal
      ...
      <ScrollView>
        <View style={styles.container}>
          <View style={styles.titleContainer}>
            ...
            <View>
              <CustomTitle>Editar Atividade</CustomTitle>
            </View>
          </View>
          ...
          <View style={styles.footerContainer}>
            ...
            <CustomButton
              ...
              size="normal"
              variant="ghost"
              onClick={handleOpenWargingModal}
            />
          </View>
        </View>
      </ScrollView>
      ...
      <CustomDialog
        ...
        message="Tem certeza que deseja cancelar esta atividade? Você não poderá mais acessá-la."
